...
if (!check_setup($prefix."evaluation",$database)) {
  ...
  $updateSQL = sprintf("CREATE TABLE `%s` (
    ...
    `evalDurationSec` int(10) DEFAULT NULL COMMENT 'Seconds spent on initial scoresheet submission',
    `evalTable` smallint(5) DEFAULT NULL COMMENT 'ID of table from judging_tables table',
    `evalFinalScore` float DEFAULT NULL COMMENT 'final, agreed upon score',
    `evalDraft` tinyint(1) DEFAULT 0 COMMENT '1=in-progress autosave draft; 0=finalized evaluation',
    `evalWaiveConsensus` char(1) DEFAULT NULL COMMENT '1=true - judge continued without waiting for another judge''s evaluation',
