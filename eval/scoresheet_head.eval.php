...
// Always show the real entry number on printed scoresheets.
// Judging numbers are internal and must not appear on entrant-facing sheets.
$entry_id = sprintf("%06s",$row_entry_info['id']);
...
// Hide Mini-BOS on entrant-facing scoresheets until places/results are released
if (($show_places) && ((!empty($row_eval['evalMiniBOS'])) || (!empty($row_eval['scoreMiniBOS'])))) $head_miniBOS = TRUE;
