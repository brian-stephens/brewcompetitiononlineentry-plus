...
$eval_place_actions .= "<select class=\"selectpicker eval-place-choose-".$tbl_id."\" data-dropdown-align-right=\"auto\" id=\"eval-place-ajax-".$row_entries['id']."\" name=\"evalPlace".$row_entries['id']."\" data-width=\"100%\" onchange=\"update_place_display('".$number."','eval-place-ajax-".$row_entries['id']."','".$tbl_id."');select_place_multi('".$ajax_url."','evalPlace','evaluation','".$row_entries['id']."','eval-place-choose-".$tbl_id."','','','','eval-place-ajax-".$row_entries['id']."');\">";
	...
	// Set Consensus (Admin)
	$set_consensus_judges = array();
	foreach ($eval_scores as $value) {
		if ($value['eid'] == $row_entries['id']) {
			$sc_judge = brewer_info($value['judge_id']);
			$sc_judge = explode("^", $sc_judge);
			$set_consensus_judges[] = array(
				"judge" => trim($sc_judge[0]." ".$sc_judge[1]),
				"score" => $value['judge_score'],
				"consensus" => $value['consensus_score']
			);
		}
	}

	$set_consensus_current = "";
	if ((!empty($assigned_score)) && (count(array_unique($assigned_score)) === 1)) $set_consensus_current = $assigned_score[0];

	$eval_place_actions .= "<div class=\"row\">";
	$eval_place_actions .= "<div class=\"col col-lg-6 col-md-7 col-sm-12\">";
	$eval_place_actions .= $label_set_consensus;
	$eval_place_actions .= "</div>";
	$eval_place_actions .= "<div class=\"col col-lg-6 col-md-5 col-sm-12\">";
	$eval_place_actions .= sprintf(
		"<button type=\"button\" class=\"btn btn-default btn-xs set-consensus-btn\" data-eid=\"%s\" data-number=\"%s\" data-current=\"%s\" data-judges=\"%s\"><i class=\"fa fa-fw fa-balance-scale\"></i> %s</button>",
		$row_entries['id'],
		htmlspecialchars($number, ENT_QUOTES, "UTF-8"),
		htmlspecialchars($set_consensus_current, ENT_QUOTES, "UTF-8"),
		htmlspecialchars(json_encode($set_consensus_judges), ENT_QUOTES, "UTF-8"),
		$label_set_consensus
	);
	$eval_place_actions .= " <span id=\"set-consensus-ajax-".$row_entries['id']."-status\"></span>";
	$eval_place_actions .= "</div>";
	$eval_place_actions .= "</div>";
...
if ($count_evals > 0) {
	...
	// Consensus/assigned score is now auto-computed as the average of every
	// judge's total for the entry, so it's always the same value for everyone -
	// just display it once at least one judge has scored.
