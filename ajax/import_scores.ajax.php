...
if ((isset($_SESSION['session_set_'.$prefix_session])) && (isset($_SESSION['loginUsername'])) && ($_SESSION['userLevel'] <= 1)) {
	...
	// Optional scoping: import only a single table, or every table within a
	// single judging session/location. table_id wins if both are sent.
	// Neither present preserves the original all-tables behavior.
	$filter_table_id = 0;
	$filter_session_id = 0;
	if ((isset($_POST['table_id'])) && (is_numeric($_POST['table_id'])) && ($_POST['table_id'] > 0)) $filter_table_id = (int) sterilize($_POST['table_id']);
	elseif ((isset($_GET['table_id'])) && (is_numeric($_GET['table_id'])) && ($_GET['table_id'] > 0)) $filter_table_id = (int) sterilize($_GET['table_id']);

	if ((isset($_POST['session_id'])) && (is_numeric($_POST['session_id'])) && ($_POST['session_id'] > 0)) $filter_session_id = (int) sterilize($_POST['session_id']);
	elseif ((isset($_GET['session_id'])) && (is_numeric($_GET['session_id'])) && ($_GET['session_id'] > 0)) $filter_session_id = (int) sterilize($_GET['session_id']);

	// Draft autosave rows are never eligible for import/consensus matching.
	$eval_import_conditions = array();
	if (check_update("evalDraft", $prefix."evaluation")) $eval_import_conditions[] = "(evalDraft <> '1' OR evalDraft IS NULL)";
	if ($filter_table_id > 0) $eval_import_conditions[] = sprintf("evalTable='%s'", $filter_table_id);
	elseif ($filter_session_id > 0) $eval_import_conditions[] = sprintf("evalTable IN (SELECT id FROM %s WHERE tableLocation='%s')", $prefix."judging_tables", $filter_session_id);

	$eval_import_where = "";
	$eval_import_where_and = "";
	if (!empty($eval_import_conditions)) {
		$eval_import_where = " WHERE ".implode(" AND ", $eval_import_conditions);
		$eval_import_where_and = " AND ".implode(" AND ", $eval_import_conditions);
	}

	// First, query DB to see if any evaluations have been recorded
	$query_eval = sprintf("SELECT * FROM %s%s",$prefix."evaluation",$eval_import_where);
	...
	// If there are records, update the scores DB table with scores recorded by judges
		...
		do {
			...
			if ((string)$scored_place === "4") $scored_place = "5";
			$scored_places[] = array(
				"scored_id" => $row_scored['eid'],
				"scored_place" => $scored_place,
		...
		*/
		...
		foreach ($eval_arr as $value) {
			...
			$query_evals = sprintf("SELECT * FROM %s WHERE eid='%s'%s",$prefix."evaluation",$value,$eval_import_where_and);
			...
			if (in_array($value,$scored_arr)) {
				...
				if ((string)$evalPlace === "4") $evalPlace = "5";
			...
			// If no record, import
				...
				if ($totalRows_evals > 1) {
					...
					$query_eval_max = sprintf("SELECT COUNT(*) as 'count' FROM %s WHERE eid='%s' AND evalFinalScore='%s'%s",$prefix."evaluation",$value,$final_score,$eval_import_where_and);
