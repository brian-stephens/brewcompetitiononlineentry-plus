...
if (session_status() == PHP_SESSION_NONE) {
    ...
    // Align PHP GC with app inactivity timeout when already known (HOSTED).
    // Non-hosted installs set this again after config.php loads below.
    if ((!empty($session_expire_after)) && (is_numeric($session_expire_after))) {
        ini_set('session.gc_maxlifetime', (string) ((int) $session_expire_after * 60));
    }
...
// Keep PHP's session garbage-collection lifetime in sync with the app timeout.
// Without this, PHP's default (often 1440s / 24 min) can wipe session data while
// the UI still shows a longer "Auto Log Out" countdown.
if ((!empty($session_expire_after)) && (is_numeric($session_expire_after))) {
    $session_expire_after = (int) $session_expire_after;
    ini_set('session.gc_maxlifetime', (string) ($session_expire_after * 60));
}
