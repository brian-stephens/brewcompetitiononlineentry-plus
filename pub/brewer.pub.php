...
if (($_SESSION['prefsProEdition'] == 0) || (($_SESSION['prefsProEdition'] == 1) && ($show_judge_steward_fields))) {
    ...
    if ($section != "step2") {
        if ((!empty($row_brewer['brewerClubs'])) && (!in_array(html_entity_decode($row_brewer['brewerClubs']),$_SESSION['club_array']))) {
            $club_other = TRUE;
        ...
        if ((!$club_other) && (!in_array(html_entity_decode($row_brewer['brewerClubs']),$_SESSION['club_array'])) && (!empty($row_brewer['brewerClubs']))) $club_alert .= sprintf("<div id=\"clubOther-warning\" class=\"alert alert-warning\"><span class=\"fa fa-exclamation-circle\"></span> <strong>%s</strong> %s %s</div>",$brewer_text_039,$brewer_text_040,$brewer_text_038);
    }

    foreach ($_SESSION['club_array'] as $club) {
        $club_selected = "";
        if ($section != "step2") {
            if ($club == html_entity_decode($row_brewer['brewerClubs'])) $club_selected = " SELECTED";
        }
        $club_options .= "<option value=\"".htmlspecialchars($club, ENT_QUOTES, 'UTF-8')."\"".$club_selected.">".htmlspecialchars($club, ENT_QUOTES, 'UTF-8')."</option>\n";
    }
    ...
    do { 
        ...
        */
        ...
        if ($row_judging3['judgingLocType'] == 2) {
            ...
            if ((((time() < $row_judging3['judgingDate']) && ($row_judging3['judgingLocHidden'] != "1"))  || (($go == "admin") && ($filter != "default"))) && (!empty($judge_avail_info) && (!empty($judge_avail_option)))) {
            ...
            if ((((time() < $row_judging3['judgingDate']) && ($row_judging3['judgingLocHidden'] != "1"))  || (($go == "admin") && ($filter != "default"))) && (!empty($steward_avail_info) && (!empty($steward_avail_option))))  {
