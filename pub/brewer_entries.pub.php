...
if ((!$show_scores) && (session_pref_enabled('prefsCoupons', 1))) $page_info1 .= sprintf("<li><strong>%s:</strong> %s</li>",$label_coupon_credits,$coupon_credits_available);
...
if (($totalRows_log > 0) && ($action != "print")) {
	...
	if (($totalRows_log - $totalRows_log_confirmed) > 0) {
		...
		$warnings .= sprintf(" %s",$brewer_entries_text_003);
		$warnings .= "</div>";
	}
	...
	if (($total_not_paid > 0) && ($judging_past > 0) && ($registration_open < 2) && (!$disable_pay)) {
...
if ($_SESSION['prefsEval'] == 1) {
	...
	$evals = array();
	// Only check evaluations for this user's entries
	$query_eval_exists = sprintf("SELECT DISTINCT eid FROM %s WHERE eid IN (SELECT id FROM %s WHERE brewBrewerID='%s')",$prefix."evaluation",$brewing_db_table,$_SESSION['user_id']);
...
if ($totalRows_log > 0) {
	...
	do {
		...
		if ($show_scores) {
			...
			if (($show_places) && (minibos_check($row_log['id'],$judging_scores_db_table))) $entry_output .= "<br>".$label_mini_bos." <i class=\"fa fa-sm fa-check text-success\"></i>";
		}
		...
		*/
		...
		// Apply Coupon
		if (session_pref_enabled('prefsCoupons', 1)) {
		if ((!$show_scores) && ($row_log['brewPaid'] != 1) && ($coupon_apply_allowed)) {
			$apply_coupon_link .= "<a class=\"hide-loader\" role=\"button\" href=\"".$base_url."includes/process.inc.php?section=list&amp;action=apply_coupon_to_entry&amp;id=".$row_log['id']."\" data-bs-toggle=\"tooltip\" data-bs-placement=\"top\" title=\"".$label_apply_coupon."\"><i class=\"fa fa-fw fa-lg fa-ticket\"></i></a>";
		}
		else {
			$apply_coupon_link .= "<a class=\"hide-loader\" role=\"button\" data-bs-toggle=\"tooltip\" data-bs-placement=\"top\" title=\"".$label_apply_coupon."\"><i class=\"fa fa-fw fa-lg fa-ticket text-muted\"></i></a>";
		}
		}
		...
		if ($print_bottle_labels) {
			...
			if ($row_log['brewPaid'] == 1) {
					...
					$print_forms_link .= $brewer_entries_text_018;
		...
		// If a judging date has not passed yet
			...
			if (!$show_scores) {
				...
				if (session_pref_enabled('prefsCoupons', 1)) $entry_output .= "<span class=\"me-1\">".$apply_coupon_link."</span>";
			}
		...
		if (($show_places) && (!empty($medal_winner))) $entry_output_cards .= "<div class=\"position-absolute top-0 start-50 translate-middle badge bg-black border border-secondary text-white rounded-pill winner-place-pill\"><span class=\"fw-normal\"> ".display_place($winner_place,2)."</span></div>";
    	...
    	*/
		...
		if ($show_scores) {
			...
			if (($show_places) && (minibos_check($row_log['id'],$judging_scores_db_table))) $entry_output_cards .= sprintf("<li><strong>%s:</strong> <i class=\"fa fa-sm fa-check text-success\"></i></li>",$label_mini_bos);
			if (($show_places) && (!empty($medal_winner))) $entry_output_cards .= sprintf("<li><strong>%s:</strong> %s</li>",str_replace("?", "", $label_winner), str_replace(":", " -", $medal_winner));
		...
		if (!$show_scores) {
			...
			// Edit Button
			$entry_output_cards .= "<div class=\"col-3\">";
		...
		$entry_output_cards .= "</div>";
			...
			if (session_pref_enabled('prefsCoupons', 1)) {
			$entry_output_cards .= "<div class=\"col-3\">";
			$entry_output_cards .= sprintf("<div class=\"text-center\">%s</div>", $apply_coupon_link);
			$entry_output_cards .= "</div>";
			}
