...
// Cap entry counts in homepage alert text (display only)
if ($section == "default") {
	if (!isset($display_total_entries)) $display_total_entries = $total_entries;
	if ($display_total_entries != $total_entries) {
		$alert_text_043 = str_replace((string)$total_entries, (string)$display_total_entries, $alert_text_043);
		$alert_text_047 = str_replace((string)$total_entries, (string)$display_total_entries, $alert_text_047);
		$alert_text_060 = str_replace((string)$total_entries, (string)$display_total_entries, $alert_text_060);
	}
}
...
if ($msg != "default") { 
  ...
  if ($section == "evaluation") {
    // msg=3 = added, msg=2 = edited (see headers.inc.php evaluation case)
    $success_msg = array(2,3);
    unset($danger_msg_alerts[0]);
  }
  ...
  if (!empty($output)) {
    ...
    // Evaluation success banners should sit in normal document flow (not position:fixed)
    // so they don't stick to the viewport while judges scroll the reconcile/dashboard pages.
    $alert_stacked = ($section == "evaluation");
    echo create_bs_alert("alert-auto-dismiss",$alert_type,"",$output,$alert_icon,"",$alert_stacked);
    ?>
    <script type="text/javascript">
    (function() {
      var el = document.getElementById("alert-auto-dismiss");
      if (!el) return;
...
if ($logged_in) { 
  ...
  if ((isset($_SESSION['contestShippingAddress'])) && ($_SESSION['prefsShipping'] == 1) && ($shipping_window_open == 2) && (!$ua) && ($section == "default") && ($judging_past > 0) && ($msg == "default")) { 
