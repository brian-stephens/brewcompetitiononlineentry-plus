	...
	if ($_SESSION['user_id'] == $value['judge_id']) {
		...
		if (!in_array($value['table'], $table_assignments_user)) {
			...
			if ($totalRows_entry > 0) {
				...
				if ((!empty($table_info)) && (isset($table_info[2]))) {
				...
				// Consensus/assigned score is auto-computed as the average of every
				// judge's total for the entry, so it's always the same value for
				// everyone once at least one other judge has scored - just display it.
				if ($totalRows_evals > 1) {
					...
					$notes_otf .= "<div style=\"margin-bottom:5px;\" class=\"text-success\">".$label_assigned_score.": ".$assigned_score[0]."</div>";
...
if (!empty($on_the_fly_display_tbody)) {
	...
	$on_the_fly_display .= "<h3 class=\"mt-5\">".$label_unassigned_eval."</h3>";
	$on_the_fly_display .= "<p>".$evaluation_info_027."</p>";
	$on_the_fly_display .= "<table id=\"table-on-the-fly\" class=\"table table-condensed table-striped table-bordered table-responsive border-dark-subtle\">";
	$on_the_fly_display .= "<thead>";
	$on_the_fly_display .= "<tr class=\"table-dark\">";
