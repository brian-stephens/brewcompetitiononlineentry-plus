...
$entry_status_body_content .= sprintf("<strong>%s</strong> &ndash; <span id=\"entry-total-count\" class=\"entry-total-count\">%s</span>",$label_total, $display_total_entries);
...
$entry_status_body_content .= "</li>";
$entry_status_body_content .= "<li>";
$entry_status_body_content .= sprintf("<strong>%s</strong> &ndash; <span id=\"entry-paid-count\" class=\"entry-paid-count\">%s</span>", $label_paid, $display_total_paid, $row_limits['prefsEntryLimitPaid']);
...
// 1 = In Progress
// 2 = Completed
// Use the configured judging window (jPrefsJudgingOpen/Closed), not individual session start times.
...
$judging_window_open = "";
$judging_window_close = "";
if ((isset($_SESSION['jPrefsJudgingOpen'])) && (!empty($_SESSION['jPrefsJudgingOpen']))) $judging_window_open = $_SESSION['jPrefsJudgingOpen'];
elseif (!empty($first_judging_date)) $judging_window_open = $first_judging_date;
if ((isset($_SESSION['jPrefsJudgingClosed'])) && (!empty($_SESSION['jPrefsJudgingClosed']))) $judging_window_close = $_SESSION['jPrefsJudgingClosed'];
elseif (!empty($last_judging_date)) $judging_window_close = $last_judging_date;

if (!empty($judging_window_open)) {

	if (time() > $judging_window_open) {

		if (empty($judging_window_close)) {
			...
			// If not, take the judging open date and add 6 hours
			else {
				if (time() < ($judging_window_open + 21600)) $judging_start = 1;
				else $judging_start = 2;
			}
			...
			if (time() < $judging_window_close) $judging_start = 1;
			else $judging_start = 2;
	...
	$judging_open_date = getTimeZoneDateTime($_SESSION['prefsTimeZone'], $judging_window_open, $_SESSION['prefsDateFormat'], $_SESSION['prefsTimeFormat'], "short", "date-time");
	$judge_open_date = getTimeZoneDateTime($_SESSION['prefsTimeZone'], $judging_window_open, "999",  $_SESSION['prefsTimeFormat'], "short", "date-no-gmt");

	if (!empty($judging_window_close)) {
		$judging_close_date = getTimeZoneDateTime($_SESSION['prefsTimeZone'], $judging_window_close, $_SESSION['prefsDateFormat'], $_SESSION['prefsTimeFormat'], "short", "date-time");
		$judge_close_date = getTimeZoneDateTime($_SESSION['prefsTimeZone'], $judging_window_close, "999",  $_SESSION['prefsTimeFormat'], "short", "date-no-gmt");
	}
