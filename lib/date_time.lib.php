...
function judging_date_return() {
	...
	// Exclude hidden sessions; they should not keep the site in a "judging upcoming/open" state.
	$query_check = sprintf("SELECT judgingDate FROM %s WHERE (judgingLocHidden IS NULL OR judgingLocHidden <> '1')", $prefix."judging_locations");
