...
function bcoem_dashboard_help($content) {
	...
	switch($content) {
		...
		$bcoem_dashboard_help_body .= "<ol>";
		$bcoem_dashboard_help_body .= "<li>If Electronic Scoresheets are enabled, individual and consensus scores are recorded by judges with their evaluations. However, please note that <strong>scores and associated places are not official until an Admin imports the matching consensus scores into the scores database</strong>. This can be done via the <a href=\"".$base_url."index.php?section=admin&amp;go=evaluation&amp;action=overview&amp;view=admin\">Administration: Evaluation Progress</a> function.";
