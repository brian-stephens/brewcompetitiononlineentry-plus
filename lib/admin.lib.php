...
function assigned_judge_details($dbTable,$brewer_db_table,$brewing_db_table,$judging_assignments_db_table,$judging_flights_db_table) {
	...
	$table_entry_totals = array();
	$flight_entry_totals = array();

	$query_assignments = sprintf("SELECT a.bid,a.assignTable,a.assignFlight,a.assignRound,b.brewerFirstName,b.brewerLastName,b.brewerJudgeRank,b.brewerJudgeMead,b.brewerJudgeCider FROM %s a LEFT JOIN %s b ON a.bid=b.uid WHERE a.assignment='J' ORDER BY b.brewerLastName,b.brewerFirstName", $judging_assignments_db_table, $brewer_db_table);
	$assignments = mysqli_query($connection,$query_assignments) or die (mysqli_error($connection));
	$row_assignments = mysqli_fetch_assoc($assignments);

	$assignment_rows = array();
	$judge_bids_by_group = array();

	if ($row_assignments) {
		do {

			$table_id = $row_assignments['assignTable'];

			// Group key: for non-queued judging, judges are grouped by table/flight/round
			// (the entries they'll actually judge together). For queued judging, the
			// group is the whole table.
			if ($_SESSION['jPrefsQueued'] == "N") $group_key = $table_id."-".$row_assignments['assignFlight']."-".$row_assignments['assignRound'];
			else $group_key = $table_id;

			if (!isset($judge_bids_by_group[$group_key])) $judge_bids_by_group[$group_key] = array();
			$judge_bids_by_group[$group_key][$row_assignments['bid']] = TRUE;

			$assignment_rows[] = $row_assignments;
	...
	if ($_SESSION['jPrefsQueued'] == "N") {
		...
		$query_flight_entry_totals = sprintf("SELECT f.flightTable,f.flightNumber,f.flightRound,COUNT(DISTINCT f.flightEntryID) AS count FROM %s f INNER JOIN %s b ON b.id=f.flightEntryID", $judging_flights_db_table, $brewing_db_table);
		if ($_SESSION['jPrefsTablePlanning'] == 0) $query_flight_entry_totals .= " WHERE b.brewReceived='1'";
		$query_flight_entry_totals .= " GROUP BY f.flightTable,f.flightNumber,f.flightRound";
		$flight_entry_total_results = mysqli_query($connection,$query_flight_entry_totals) or die (mysqli_error($connection));
		$row_flight_entry_totals = mysqli_fetch_assoc($flight_entry_total_results);

		if ($row_flight_entry_totals) {
			do {
				$flight_key = $row_flight_entry_totals['flightTable']."-".$row_flight_entry_totals['flightNumber']."-".$row_flight_entry_totals['flightRound'];
				$flight_entry_totals[$flight_key] = $row_flight_entry_totals['count'];
			} while ($row_flight_entry_totals = mysqli_fetch_assoc($flight_entry_total_results));
		}

	}

	foreach ($assignment_rows as $row_assignments) {

			$bid = $row_assignments['bid'];
			$table_id = $row_assignments['assignTable'];
			$assignment_key = $bid."-".$table_id."-".$row_assignments['assignFlight']."-".$row_assignments['assignRound'];

			if (!isset($assignment_keys[$assignment_key])) {

				$assignment_keys[$assignment_key] = TRUE;
				$entry_total = 0;

				if ($_SESSION['jPrefsQueued'] == "N") {
					$group_key = $table_id."-".$row_assignments['assignFlight']."-".$row_assignments['assignRound'];
					if (isset($flight_entry_totals[$group_key])) $entry_total = $flight_entry_totals[$group_key];
				}

				else {
					$group_key = $table_id;
					if (!isset($table_entry_totals[$table_id])) $table_entry_totals[$table_id] = get_table_info("1","count_total",$table_id,$dbTable,"default");
					$entry_total = $table_entry_totals[$table_id];
				}

				// Judges at a table/flight typically work in pairs (a "judge pair").
				// An odd judge out joins an existing pair rather than judging solo,
				// so the number of judging groups is the number of judges divided by
				// two, rounded down (minimum of one group).
				$judges_in_group = 1;
				if (isset($judge_bids_by_group[$group_key])) $judges_in_group = count($judge_bids_by_group[$group_key]);
				$judge_groups = max(1,floor($judges_in_group / 2));

				if ($judge_groups > 0) $entry_total = ceil($entry_total / $judge_groups);

				if (!isset($judge_details['entry_totals'][$bid])) $judge_details['entry_totals'][$bid] = 0;
				$judge_details['entry_totals'][$bid] += $entry_total;

			}

			if (!isset($judge_details['by_table'][$table_id][$bid])) {
				$judge_details['by_table'][$table_id][$bid] = array(
					"first_name" => $row_assignments['brewerFirstName'],
					"last_name" => $row_assignments['brewerLastName'],
					"rank" => $row_assignments['brewerJudgeRank'],
					"mead" => $row_assignments['brewerJudgeMead'],
					"cider" => $row_assignments['brewerJudgeCider']
				);
			}
...
function assigned_steward_details($brewer_db_table,$judging_assignments_db_table) {

	include (CONFIG.'config.php');
	mysqli_select_db($connection,$database);

	$steward_details = array(
		"by_table" => array()
	);

	$query_assignments = sprintf("SELECT a.bid,a.assignTable,b.brewerFirstName,b.brewerLastName FROM %s a LEFT JOIN %s b ON a.bid=b.uid WHERE a.assignment='S' ORDER BY b.brewerLastName,b.brewerFirstName", $judging_assignments_db_table, $brewer_db_table);
	$assignments = mysqli_query($connection,$query_assignments) or die (mysqli_error($connection));
	$row_assignments = mysqli_fetch_assoc($assignments);
	...
	if ($row_assignments) {
		do {
			...
			if (!isset($steward_details['by_table'][$table_id][$bid])) {
				$steward_details['by_table'][$table_id][$bid] = array(
					"first_name" => $row_assignments['brewerFirstName'],
					"last_name" => $row_assignments['brewerLastName']
				);
			}
...
function unassign($bid,$location,$round,$tid) {
	...
	// Only return another table's assignment in this round/location (optional "move" checkbox).
	// Clearing this table is handled separately by deleting bid+assignTable on save.
	$query_assignments = sprintf("SELECT id FROM %s WHERE bid='%s' AND assignRound='%s' AND assignLocation='%s' AND assignTable!='%s' LIMIT 1", $prefix."judging_assignments", $bid, $round, $location, $tid);
	$assignments = mysqli_query($connection,$query_assignments) or die (mysqli_error($connection));
	$row_assignments = mysqli_fetch_assoc($assignments);
...
function assign_to_table($tid,$bid,$filter,$total_flights,$round,$location,$table_styles,$queued,$random,$ind_aff_flag) {
	...
	// This table's assignment id (for role updates); do not reuse other-table move id.
	require(CONFIG.'config.php');
	mysqli_select_db($connection,$database);
	$this_assign_id = 0;
	$query_this_assign = sprintf("SELECT id FROM %s WHERE bid='%s' AND assignTable='%s' AND assignRound='%s' AND assignLocation='%s' LIMIT 1", $prefix."judging_assignments", $bid, $tid, $round, $location);
	$this_assign = mysqli_query($connection,$query_this_assign) or die (mysqli_error($connection));
	$row_this_assign = mysqli_fetch_assoc($this_assign);
	if (!empty($row_this_assign)) $this_assign_id = $row_this_assign['id'];
	...
	$r .= '<input type="hidden" name="id'.$random.'" value="'.$this_assign_id.'"/>';
	...
	if ($unassign > 0) {
		...
		// Check to see if the participant is already assigned to another table in this round.
		// If so, offer an optional move instead of adding a second assignment.
		...
		$r .= ' Move from their other table assignment instead of adding another</label>';
		$r .= '</div>';
		$r .= '</div>';
...
function judge_alert($round,$bid,$tid,$location,$likes,$dislikes,$table_styles,$id,$ind_aff_flag) {
	...
	if (table_round($tid,$round)) {
		...
		if ($unavailable) {
		    ...
		    if ($ind_aff_flag) $r .= "<span class=\"text-purple\"><span class=\"fa fa-check\"></span> <strong>Also assigned elsewhere.</strong> Participant is assigned to another table in this round. Assigning here will add this table unless you choose to move them.</span><br><span class=\"fa fa-exclamation-circle\"></span> <strong>Conflict.</strong> Participant has reported an affiliation with one or more participants who have entries at this table. <strong>You are able to assign them to this table if you wish, but do so with caution and due diligence by checking their affiliation(s) via Manage Entries.</strong>";
		    else $r .= "<span class=\"text-purple\"><span class=\"fa fa-check\"></span> <strong>Also assigned elsewhere.</strong> Participant is assigned to another table in this round. Assigning here will add this table unless you choose to move them.</span>";
