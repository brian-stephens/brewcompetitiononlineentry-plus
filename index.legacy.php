...
<div class="<?php if (($go == "evaluation") && (($action == "add") || ($action == "edit"))) echo "container"; else echo $container_main; ?>">
    ...
    <!-- Admin Page - full width of viewport -->
        ...
        <?php
            ...
            if (($go == "judging_analytics") && ($_SESSION['jPrefsJudgeStats'] == "Y")) include (ADMIN.'judging_analytics.admin.php');
            if ($go == "participants") include (ADMIN.'participants.admin.php');
            if ($go == "entries") {
                // V3 - use legacy add/edit entry builds for Admin
                if (($action == "edit") || ($action == "add")) include (SECTIONS.'brew.sec.php');
                else include (ADMIN.'entries.admin.php');
            }

            // V3 - use legacy register/edit user builds for Admin
            if ($go == "brewer") include (SECTIONS.'brewer.sec.php');
            ...
            if ($go == "coupons") {
                if (session_pref_enabled('prefsCoupons', 1)) include (ADMIN.'coupons.admin.php');
                else echo "<div class=\"alert alert-danger\"><p><strong>Access denied.</strong></p></div>";
            }

            if (($_SESSION['prefsEval'] == 1) && ($go == "evaluation")) {
                if ($action == "overview") {
                    if (session_pref_enabled('prefsEvalAdminTools', 1)) include (EVALS.'overview.eval.php');
                    else echo "<div class=\"alert alert-danger\"><p><strong>Access denied.</strong></p></div>";
                }
                if ($action == "entrant_tracker") {
                    if (session_pref_enabled('prefsEvalAdminTools', 1)) include (EVALS.'entrant_tracker.eval.php');
                    else echo "<div class=\"alert alert-danger\"><p><strong>Access denied.</strong></p></div>";
                }
                if ($action == "judge_view") {
                    if (session_pref_enabled('prefsEvalAdminTools', 1)) include (EVALS.'judge_view.eval.php');
                    else echo "<div class=\"alert alert-danger\"><p><strong>Access denied.</strong></p></div>";
                }
