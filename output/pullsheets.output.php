		...
		if ($row_assignments) {
			...
			do {
				...
				if ($show_table) {
								...
								do {
									...
									if (!empty($row_entries['brewCategorySort'])) {
										...
										if ($display_entry) {
											...
											else $table_flight_tbody .= $style." ".$row_entries['brewStyle'];
											$table_flight_tbody .= "</td>";
											...
											if ((!empty($row_entries['brewPouring'])) && ((!empty($row_entries['brewStyleType'])) && ($row_entries['brewStyleType'] == 1))) {
												...
												if ((isset($pouring_arr['pouring_rouse'])) && ($pouring_arr['pouring_rouse'] == $label_yes)) $table_flight_tbody .= "<strong>".$label_rouse_yeast.":</strong> ".$pouring_arr['pouring_rouse']."<br>";
											}
					...
					$table_info_header .= $table_info[1];
		...
		do {
			...
			if (($row_table_round['count'] >= 1) || ($round == "default")) {
				...
				if ($entry_count > 0) {
					...
					$table_info_header .= sprintf("%s <small><em>%s</em></small>",$row_tables['tableName'],$label_additional_info);
...
if ($go == "mini_bos") {
	...
	if ($totalRows_entries_mini > 0) {
		...
		do {
			...
			else $table_flight_tbody .= $style." ".$row_entries_mini['brewStyle']; 
			$table_flight_tbody .= "</td>";
			...
			*/
			...
			if ((!empty($row_entries_mini['brewPouring'])) && ((!empty($row_entries_mini['brewStyleType'])) && ($row_entries_mini['brewStyleType'] == 1))) {
				...
				if ((isset($pouring_arr['pouring_rouse'])) && ($pouring_arr['pouring_rouse'] == $label_yes)) $table_flight_tbody .= "<li><strong>".$label_rouse_yeast.":</strong> ".$pouring_arr['pouring_rouse']."</li>";
			}
			...
			if ($totalRows_bos > 0) {
				...
				do {
					...
					if (!empty($row_bos['brewCategorySort'])) {
						...
						else $table_flight_tbody .= $style." ".$row_bos['brewStyle'];
						$table_flight_tbody .= "</td>";
		...
		if ($tables_all) {
			...
			do {
				...
				if (($row_table_round['count'] >= 1) || ($round == "default")) {
					...
					$table_info_header .= "<div class=\"page-header\">";
					$table_info_header .= "<h1>";
					$table_info_header .= $row_tables['tableName'];
			...
			if ($filter == "mini_bos") {
				...
				do {
					...
					*/
					...
					// $table_flight .= "<h3>".sprintf("%s - %s %s, %s %s",$row_tables['tableName'],$label_flight,$i,$label_round,$row_round_check['flightRound'])."</h3>";
					*/
