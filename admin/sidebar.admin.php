...
if ($logged_in) {
	...
	if ($_SESSION['prefsEval'] == 1) {
		...
		$eval_count_href = $base_url."index.php?section=admin&amp;go=evaluation&amp;filter=default&amp;view=admin";
		if (session_pref_enabled('prefsEvalAdminTools', 1)) $eval_count_href = $base_url."index.php?section=admin&amp;go=evaluation&amp;action=overview&amp;view=admin";
		$admin_sidebar_body .= "<span class=\"pull-right\"><small><i class=\"fa fa-xs fa-circle text-success eval-update-indicator\"></i></small> <a href=\"".$eval_count_href."\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Total evaluations\"><span id=\"admin-dashboard-evaluation-count-total\">".get_evaluation_count('total')."</span></a> / <a href=\"".$eval_count_href."\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Entries with evaluations\"><span id=\"admin-dashboard-evaluation-count\">".get_evaluation_count('unique')."</span></a></span>";
		$admin_sidebar_body .= "</div>";

		if ($_SESSION['jPrefsJudgeStats'] == "Y") {

			$sidebar_judging_analytics = get_all_judges_analytics_stats();

			$admin_sidebar_body .= "<div class=\"bcoem-sidebar-panel\">";
			$admin_sidebar_body .= "<strong class=\"text-info me-1\">Judge Analytics</strong>";
			$admin_sidebar_body .= "<span class=\"pull-right\"><a href=\"".$base_url."index.php?section=admin&amp;go=judging_analytics\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"View Judge Analytics report\">".$sidebar_judging_analytics['competition']['comp_avg_words']." words";
			if ($sidebar_judging_analytics['competition']['comp_duration_count'] > 0) $admin_sidebar_body .= " / ".gmdate("i:s",$sidebar_judging_analytics['competition']['comp_avg_duration'])." min:sec";
			$admin_sidebar_body .= " avg.</a></span>";
			$admin_sidebar_body .= "</div>";
