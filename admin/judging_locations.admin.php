...
if ($section != "step5") {
	...
	if (($action == "default") && ($totalRows_judging_locs > 0)) {
		...
		do {
			...
			$output_datatables_body .= "<td>".$row_judging_locs['judgingLocName'];
			if ($row_judging_locs['judgingLocHidden'] == "1") $output_datatables_body .= " <span class=\"label label-default\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"This session is hidden from all front end views. It can still be assigned to in the backend.\"><span class=\"fa fa-eye-slash\"></span> Hidden</span>";
			if ((isset($row_judging_locs['judgingLocComplete'])) && ($row_judging_locs['judgingLocComplete'] == "1")) $output_datatables_body .= " <span class=\"label label-success\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"This session is marked complete and is hidden from judge dashboards.\"><span class=\"fa fa-check\"></span> Complete</span>";
...
<div class="form-group">
    <label for="judgingLocHidden" class="col-lg-2 col-md-3 col-sm-4 col-xs-12 control-label">Hide from Front End</label>
    <div class="col-lg-6 col-md-6 col-sm-8 col-xs-12">
        <div class="checkbox">
            <label>
                <input type="checkbox" id="judgingLocHidden" name="judgingLocHidden" value="1" <?php if (($action == "edit") && ($row_judging['judgingLocHidden'] == "1")) echo "CHECKED"; ?>> Hide this session from all front end (public) views.
            </label>
        </div>
        <span class="help-block">This session will not appear on the main page or be selectable when judges/stewards self-register. It can still be assigned to in the admin backend.</span>
    </div>
</div>

<div class="form-group">
    <label for="judgingLocComplete" class="col-lg-2 col-md-3 col-sm-4 col-xs-12 control-label">Mark Complete</label>
    <div class="col-lg-6 col-md-6 col-sm-8 col-xs-12">
        <div class="checkbox">
            <label>
                <input type="checkbox" id="judgingLocComplete" name="judgingLocComplete" value="1" <?php if (($action == "edit") && (isset($row_judging['judgingLocComplete'])) && ($row_judging['judgingLocComplete'] == "1")) echo "CHECKED"; ?>> Mark this session as complete.
            </label>
        </div>
        <span class="help-block">Completed sessions are removed from judge dashboards so judges only see sessions that still need their attention. Admins can still manage the session and its evaluations in the backend.</span>
    </div>
</div>
