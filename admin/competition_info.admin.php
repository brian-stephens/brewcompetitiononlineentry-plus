...
$extra_ack_slot_max = 5;
$extra_acks_edit = array();
$extra_ack_enabled = 0;
if (($section != "step4") && (isset($row_contest_info))) {
    if (isset($row_contest_info['contestExtraAck'])) $extra_ack_enabled = $row_contest_info['contestExtraAck'];
    elseif (isset($row_contest_info['contestMichiganAck'])) $extra_ack_enabled = $row_contest_info['contestMichiganAck'];
    if ((isset($row_contest_info['contestExtraAcks'])) && (!empty($row_contest_info['contestExtraAcks']))) {
        $decoded_extra_acks = json_decode($row_contest_info['contestExtraAcks'], true);
        if (is_array($decoded_extra_acks)) $extra_acks_edit = $decoded_extra_acks;
    }
    if (empty($extra_acks_edit)) {
        $legacy_title = "";
        $legacy_text = "";
        $legacy_checkbox = "";
        if ((isset($row_contest_info['contestMichiganBannerTitle'])) && (!empty($row_contest_info['contestMichiganBannerTitle']))) $legacy_title = $row_contest_info['contestMichiganBannerTitle'];
        if ((isset($row_contest_info['contestMichiganBannerText'])) && (!empty($row_contest_info['contestMichiganBannerText']))) $legacy_text = $row_contest_info['contestMichiganBannerText'];
        if ((isset($row_contest_info['contestMichiganCheckboxText'])) && (!empty($row_contest_info['contestMichiganCheckboxText']))) $legacy_checkbox = $row_contest_info['contestMichiganCheckboxText'];
        if ((!empty($legacy_title)) || (!empty($legacy_text)) || (!empty($legacy_checkbox))) {
            $extra_acks_edit[] = array(
                'title' => $legacy_title,
                'text' => $legacy_text,
...
while (count($extra_acks_edit) < $extra_ack_slot_max) {
    $extra_acks_edit[] = array('title' => '', 'text' => '', 'checkbox' => '');
}
...
<div class="form-group">
	...
	getTimeZoneDateTime($_SESSION['prefsTimeZone'], $row_contest_dates['contestEntryOpen'], $_SESSION['prefsDateFormat'],  $_SESSION['prefsTimeFormat'], "system", "date-time-system"); ?>" placeholder="<?php echo $current_date." ".$current_time; ?>" required>
        	<span class="input-group-addon" id="contestEntryOpen-addon2" data-tooltip="true" title="<?php echo $form_required_fields_02; ?>"><span class="fa fa-star"></span></span>
        </div>
    </div>
</div>
...
<div class="form-group">
	...
	getTimeZoneDateTime($_SESSION['prefsTimeZone'], $row_contest_dates['contestEntryDeadline'], $_SESSION['prefsDateFormat'],  $_SESSION['prefsTimeFormat'], "system", "date-time-system"); ?>" placeholder="<?php echo $current_date." ".$current_time; ?>" required>
        	<span class="input-group-addon" id="contestEntryDeadline-addon2" data-tooltip="true" title="<?php echo $form_required_fields_02; ?>"><span class="fa fa-star"></span></span>
...
<h3>Extra Acknowledgments</h3>
<div class="form-group">
    <label for="contestExtraAck" class="col-lg-2 col-md-3 col-sm-4 col-xs-12 control-label">Extra Acknowledgments</label>
    <div class="col-lg-6 col-md-6 col-sm-8 col-xs-12">
        <div class="input-group">
            <label class="radio-inline">
                <input type="radio" name="contestExtraAck" value="1" id="contestExtraAck_1" <?php if ($extra_ack_enabled == "1") echo "CHECKED"; ?> /> Enable
            </label>
            <label class="radio-inline">
                <input type="radio" name="contestExtraAck" value="0" id="contestExtraAck_0" <?php if (($section == "step4") || ($extra_ack_enabled != "1")) echo "CHECKED"; ?> /> Disable
            </label>
        </div>
        <span id="helpBlock" class="help-block">When enabled, public registration shows a banner and requires a checkbox for each acknowledgment below. Use this for residency, rules, or any other confirmation you need. Leave a field blank to use the default wording. Empty acknowledgments are skipped. You can add up to five.</span>
    </div>
</div>
<?php for ($extra_ack_i = 0; $extra_ack_i < $extra_ack_slot_max; $extra_ack_i++) {
    $extra_ack_item = $extra_acks_edit[$extra_ack_i];
    $extra_ack_title = isset($extra_ack_item['title']) ? $extra_ack_item['title'] : "";
    $extra_ack_text = isset($extra_ack_item['text']) ? $extra_ack_item['text'] : "";
    $extra_ack_checkbox = isset($extra_ack_item['checkbox']) ? $extra_ack_item['checkbox'] : "";
?>
<div class="extra-ack-block" data-ack-index="<?php echo $extra_ack_i; ?>">
    <h4>Acknowledgment <?php echo ($extra_ack_i + 1); ?></h4>
    <div class="form-group">
        <label for="contestExtraAckTitle<?php echo $extra_ack_i; ?>" class="col-lg-2 col-md-3 col-sm-4 col-xs-12 control-label">Banner Title</label>
        <div class="col-lg-6 col-md-6 col-sm-8 col-xs-12">
            <input class="form-control" id="contestExtraAckTitle<?php echo $extra_ack_i; ?>" name="contestExtraAckTitle[]" type="text" maxlength="255" value="<?php echo htmlspecialchars($extra_ack_title, ENT_QUOTES, 'UTF-8'); ?>" placeholder="<?php echo htmlspecialchars($register_text_053, ENT_QUOTES, 'UTF-8'); ?>">
        </div>
    </div>
    <div class="form-group">
        <label for="contestExtraAckText<?php echo $extra_ack_i; ?>" class="col-lg-2 col-md-3 col-sm-4 col-xs-12 control-label">Banner Text</label>
        <div class="col-lg-6 col-md-6 col-sm-8 col-xs-12">
            <textarea id="contestExtraAckText<?php echo $extra_ack_i; ?>" class="form-control" name="contestExtraAckText[]" rows="3" placeholder="<?php echo htmlspecialchars($register_text_054, ENT_QUOTES, 'UTF-8'); ?>"><?php echo htmlspecialchars($extra_ack_text, ENT_QUOTES, 'UTF-8'); ?></textarea>
        </div>
    </div>
    <div class="form-group">
        <label for="contestExtraAckCheckbox<?php echo $extra_ack_i; ?>" class="col-lg-2 col-md-3 col-sm-4 col-xs-12 control-label">Checkbox Label</label>
        <div class="col-lg-6 col-md-6 col-sm-8 col-xs-12">
            <textarea id="contestExtraAckCheckbox<?php echo $extra_ack_i; ?>" class="form-control" name="contestExtraAckCheckbox[]" rows="3" placeholder="<?php echo htmlspecialchars($register_text_055, ENT_QUOTES, 'UTF-8'); ?>"><?php echo htmlspecialchars($extra_ack_checkbox, ENT_QUOTES, 'UTF-8'); ?></textarea>
        </div>
    </div>
</div>
<?php } ?>
<div class="form-group" id="add-extra-ack-wrap">
    <div class="col-lg-offset-2 col-md-offset-3 col-sm-offset-4 col-lg-6 col-md-6 col-sm-8 col-xs-12">
        <button type="button" class="btn btn-sm btn-primary" id="add-extra-ack-btn">Add Another Acknowledgment</button>
    </div>
</div>
<script>
$(document).ready(function() {
    $(".extra-ack-block").each(function() {
        var idx = parseInt($(this).attr("data-ack-index"), 10);
        if (idx > 0) {
            var has = false;
            $(this).find("input[type=text], textarea").each(function() {
                if ($.trim($(this).val()) !== "") has = true;
            });
            if (!has) $(this).hide();
        }
    });
    function extraAckHiddenCount() {
        return $(".extra-ack-block:hidden").length;
    }
    if (extraAckHiddenCount() === 0) $("#add-extra-ack-btn").hide();
    $("#add-extra-ack-btn").click(function() {
        var $hidden = $(".extra-ack-block:hidden").first();
        if ($hidden.length) $hidden.show();
        if (extraAckHiddenCount() === 0) $(this).hide();
    });
});
</script>
