...
if (($action == "default") && ($filter == "default")) {
    ...
    // Judge/steward pools — match Assign Judges/Stewards (staff_* only), filtered by Y-{location} prefs
    $judge_prefs_by_location = array();
    $steward_prefs_by_location = array();
    $judge_assigned_by_loc_round = array();
    $steward_assigned_by_loc_round = array();
    $table_flight_rounds = array();

    $query_judge_avail = sprintf("SELECT a.brewerJudgeLocation FROM %s a, %s b WHERE b.staff_judge='1' AND a.uid = b.uid",$prefix."brewer",$prefix."staff");
    $judge_avail_result = mysqli_query($connection,$query_judge_avail);
    $row_judge_avail = mysqli_fetch_assoc($judge_avail_result);

    if ($row_judge_avail) {

        do {

            if (empty($row_judge_avail['brewerJudgeLocation'])) continue;
            $judge_locs = explode(",",$row_judge_avail['brewerJudgeLocation']);
            foreach ($judge_locs as $judge_loc) {
                $judge_loc = trim($judge_loc);
                if (strpos($judge_loc,"Y-") === 0) {
                    $judge_loc_id = substr($judge_loc,2);
                    if (!isset($judge_prefs_by_location[$judge_loc_id])) $judge_prefs_by_location[$judge_loc_id] = 0;
                    $judge_prefs_by_location[$judge_loc_id]++;
                }
            }

        } while ($row_judge_avail = mysqli_fetch_assoc($judge_avail_result));

    }

    $query_steward_avail = sprintf("SELECT a.brewerStewardLocation FROM %s a, %s b WHERE b.staff_steward='1' AND a.uid = b.uid",$prefix."brewer",$prefix."staff");
    $steward_avail_result = mysqli_query($connection,$query_steward_avail);
    $row_steward_avail = mysqli_fetch_assoc($steward_avail_result);

    if ($row_steward_avail) {

        do {

            if (empty($row_steward_avail['brewerStewardLocation'])) continue;
            $steward_locs = explode(",",$row_steward_avail['brewerStewardLocation']);
            foreach ($steward_locs as $steward_loc) {
                $steward_loc = trim($steward_loc);
                if (strpos($steward_loc,"Y-") === 0) {
                    $steward_loc_id = substr($steward_loc,2);
                    if (!isset($steward_prefs_by_location[$steward_loc_id])) $steward_prefs_by_location[$steward_loc_id] = 0;
                    $steward_prefs_by_location[$steward_loc_id]++;
                }
            }

        } while ($row_steward_avail = mysqli_fetch_assoc($steward_avail_result));

    }

    // Distinct assignees per location + round (same scope as unavailable())
    $query_judge_assign_by_round = sprintf("SELECT assignLocation, assignRound, COUNT(DISTINCT bid) AS count FROM %s WHERE assignment='J' GROUP BY assignLocation, assignRound",$prefix."judging_assignments");
    $judge_assign_by_round = mysqli_query($connection,$query_judge_assign_by_round);
    $row_judge_assign_by_round = mysqli_fetch_assoc($judge_assign_by_round);

    if ($row_judge_assign_by_round) {

        do {

            $judge_assigned_by_loc_round[$row_judge_assign_by_round['assignLocation']][$row_judge_assign_by_round['assignRound']] = (int)$row_judge_assign_by_round['count'];

        } while ($row_judge_assign_by_round = mysqli_fetch_assoc($judge_assign_by_round));

    }

    $query_steward_assign_by_round = sprintf("SELECT assignLocation, assignRound, COUNT(DISTINCT bid) AS count FROM %s WHERE assignment='S' GROUP BY assignLocation, assignRound",$prefix."judging_assignments");
    $steward_assign_by_round = mysqli_query($connection,$query_steward_assign_by_round);
    $row_steward_assign_by_round = mysqli_fetch_assoc($steward_assign_by_round);

    if ($row_steward_assign_by_round) {

        do {

            $steward_assigned_by_loc_round[$row_steward_assign_by_round['assignLocation']][$row_steward_assign_by_round['assignRound']] = (int)$row_steward_assign_by_round['count'];

        } while ($row_steward_assign_by_round = mysqli_fetch_assoc($steward_assign_by_round));

    }

    // Primary round per table (matches Assign Judges: highest flightRound for the table)
    $query_table_rounds = sprintf("SELECT flightTable, MAX(flightRound) AS flightRound FROM %s GROUP BY flightTable",$prefix."judging_flights");
    $table_rounds_result = mysqli_query($connection,$query_table_rounds);
    $row_table_rounds = mysqli_fetch_assoc($table_rounds_result);

    if ($row_table_rounds) {

        do {

            if (!empty($row_table_rounds['flightRound'])) $table_flight_rounds[$row_table_rounds['flightTable']] = $row_table_rounds['flightRound'];

        } while ($row_table_rounds = mysqli_fetch_assoc($table_rounds_result));
    ...
    $judge_details = assigned_judge_details($dbTable,$brewer_db_table,$brewing_db_table,$judging_assignments_db_table,$judging_flights_db_table);
    $steward_details = assigned_steward_details($brewer_db_table,$judging_assignments_db_table);
    ...
    if ($totalRows_tables > 0) {
        ...
        do {
            ...
            $table_filter_locations[$row_tables['tableLocation']] = $table_location;
            ...
            // Available = pool with Y-location pref minus distinct assignees at this location + table round
            $table_round = 1;
            if ((!empty($table_flight_rounds[$row_tables['id']])) && (is_numeric($table_flight_rounds[$row_tables['id']]))) $table_round = $table_flight_rounds[$row_tables['id']];
            $table_location_round_class = $table_location_class."-r".$table_round;

            $judge_prefs_count = 0;
            $steward_prefs_count = 0;
            if (!empty($judge_prefs_by_location[$row_tables['tableLocation']])) $judge_prefs_count = (int)$judge_prefs_by_location[$row_tables['tableLocation']];
            if (!empty($steward_prefs_by_location[$row_tables['tableLocation']])) $steward_prefs_count = (int)$steward_prefs_by_location[$row_tables['tableLocation']];

            $judge_assigned_count = 0;
            $steward_assigned_count = 0;
            if ((!empty($judge_assigned_by_loc_round[$row_tables['tableLocation']])) && (!empty($judge_assigned_by_loc_round[$row_tables['tableLocation']][$table_round]))) $judge_assigned_count = (int)$judge_assigned_by_loc_round[$row_tables['tableLocation']][$table_round];
            if ((!empty($steward_assigned_by_loc_round[$row_tables['tableLocation']])) && (!empty($steward_assigned_by_loc_round[$row_tables['tableLocation']][$table_round]))) $steward_assigned_count = (int)$steward_assigned_by_loc_round[$row_tables['tableLocation']][$table_round];

            $judge_avail = $judge_prefs_count - $judge_assigned_count;
            if ($judge_avail < 0) $judge_avail = 0;
            $steward_avail = $steward_prefs_count - $steward_assigned_count;
            if ($steward_avail < 0) $steward_avail = 0;
            ...
            $assigned_judge_details = "";

            if (!empty($judge_details['by_table'][$row_tables['id']])) {

                $assigned_judge_details .= "<div class=\"hidden-print\"><button type=\"button\" class=\"btn btn-link btn-xs\" data-toggle=\"collapse\" data-target=\"#assigned-judges-".$row_tables['id']."\" aria-expanded=\"false\" aria-controls=\"assigned-judges-".$row_tables['id']."\"><i class=\"fa fa-users\"></i> View judges</button>";
                $assigned_judge_details .= "<div class=\"collapse assigned-judge-details\" id=\"assigned-judges-".$row_tables['id']."\"><ul class=\"list-unstyled small\" style=\"margin: 5px 0 0 15px;\">";

                foreach ($judge_details['by_table'][$row_tables['id']] as $judge_bid => $judge) {
                    $judge_name = trim($judge['first_name']." ".$judge['last_name']);
                    if (empty($judge_name)) $judge_name = "Unknown Judge";
                    $judge_credentials = array();
                    $judge_rank_array = explode(",",$judge['rank']);
                    $judge_rank = trim($judge_rank_array[0]);
                    if (!empty($judge_rank)) $judge_credentials[] = bjcp_rank($judge_rank,2);
                    foreach (array_slice($judge_rank_array,1) as $judge_designation) {
                        $judge_designation = trim($judge_designation);
                        if (!empty($judge_designation)) $judge_credentials[] = $judge_designation;
                    }
                    if ($judge['mead'] == "Y") $judge_credentials[] = "Certified Mead Judge";
                    if ($judge['cider'] == "Y") $judge_credentials[] = "Certified Cider Judge";
                    $judge_entry_total = 0;
                    if (isset($judge_details['entry_totals'][$judge_bid])) $judge_entry_total = $judge_details['entry_totals'][$judge_bid];
                    $judge_entry_label = ($judge_entry_total == 1) ? " entry" : " entries";
                    $assigned_judge_details .= "<li>".htmlspecialchars($judge_name,ENT_QUOTES,"UTF-8");
                    if (!empty($judge_credentials)) $assigned_judge_details .= " <span class=\"text-muted\">&mdash; ".htmlspecialchars(implode(", ",$judge_credentials),ENT_QUOTES,"UTF-8")."</span>";
                    $assigned_judge_details .= " (".$judge_entry_total.$judge_entry_label.")</li>";
                }

                $assigned_judge_details .= "</ul></div></div>";

            }

                if ($dbTable == "default") $assigned_judges .= "<button class=\"btn-link\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Delete all judge assignments for this table.\" onclick=\"purge_data('".$ajax_url."','purge','judge-assignments','table-admin','delete-judges-".$row_tables['id']."','delete-judges-".$table_location_round_class."');\"><i class=\"text-danger fas fa-lg fa-minus-circle\"></i></button><div><span class=\"hidden\" id=\"delete-judges-".$row_tables['id']."-status\"></span><span class=\"hidden\" id=\"delete-judges-".$row_tables['id']."-status-msg\"></span></div>";
            $assigned_judges .= $assigned_judge_details;
            
            $assigned_stewards = assigned_stewards($row_tables['id'],$dbTable,$judging_assignments_db_table);
            $assigned_steward_details = "";

            if (!empty($steward_details['by_table'][$row_tables['id']])) {

                $assigned_steward_details .= "<div class=\"hidden-print\"><button type=\"button\" class=\"btn btn-link btn-xs\" data-toggle=\"collapse\" data-target=\"#assigned-stewards-".$row_tables['id']."\" aria-expanded=\"false\" aria-controls=\"assigned-stewards-".$row_tables['id']."\"><i class=\"fa fa-users\"></i> View stewards</button>";
                $assigned_steward_details .= "<div class=\"collapse assigned-steward-details\" id=\"assigned-stewards-".$row_tables['id']."\"><ul class=\"list-unstyled small\" style=\"margin: 5px 0 0 15px;\">";

                foreach ($steward_details['by_table'][$row_tables['id']] as $steward_bid => $steward) {
                    $steward_name = trim($steward['first_name']." ".$steward['last_name']);
                    if (empty($steward_name)) $steward_name = "Unknown Steward";
                    $assigned_steward_details .= "<li>".htmlspecialchars($steward_name,ENT_QUOTES,"UTF-8")."</li>";
                }

                $assigned_steward_details .= "</ul></div></div>";

            }

                if ($dbTable == "default") $assigned_stewards .= "<button class=\"btn-link\" data-toggle=\"tooltip\" data-placement=\"top\" title=\"Delete all steward assignments for this table.\" onclick=\"purge_data('".$ajax_url."','purge','steward-assignments','table-admin','delete-stewards-".$row_tables['id']."','delete-stewards-".$table_location_round_class."');\"><i class=\"text-danger fas fa-lg fa-minus-circle\"></i></button><div><span class=\"hidden\" id=\"delete-stewards-".$row_tables['id']."-status\"></span><span class=\"hidden\" id=\"delete-stewards-".$row_tables['id']."-status-msg\"></span></div>";
            $assigned_stewards .= $assigned_steward_details;
            ...
            $manage_tables_default_tbody .= "<tr data-location-id=\"".htmlspecialchars($row_tables['tableLocation'],ENT_QUOTES,"UTF-8")."\">";
            $manage_tables_default_tbody .= "<td>".$row_tables['tableNumber']."</td>";
            $manage_tables_default_tbody .= "<td>".$row_tables['tableName']."</td>";
            $manage_tables_default_tbody .= "<td class=\"judging-table-styles\">".rtrim($styles, ",&nbsp;")."</td>";
            ...
            $manage_tables_default_tbody .= "\n\n<td class=\"hidden-xs hidden-sm judging-table-judges\">".$assigned_judges."<div class=\"text-muted small\">[<span id=\"delete-judges-".$row_tables['id']."-all-count\" class=\"delete-judges-".$table_location_round_class."\">".$judge_avail."</span> available]</div></td>";
            $manage_tables_default_tbody .= "\n\n<td class=\"hidden-xs hidden-sm\">".$assigned_stewards."<div class=\"text-muted small\">[<span id=\"delete-stewards-".$row_tables['id']."-all-count\" class=\"delete-stewards-".$table_location_round_class."\">".$steward_avail."</span> available]</div></td>";
            if (($totalRows_judging > 1) && ($dbTable == "default")) $manage_tables_default_tbody .= "<td class=\"hidden-xs hidden-sm\">".$table_location."</td>";
            

            // Set js cookies of initial judge / steward available counts for each location + round
            $table_location_js_cookies[] .= "Cookies.set('delete-stewards-".$table_location_round_class."', '".$steward_avail."', { expires: 1 });";
            $table_location_js_cookies[] .= "Cookies.set('delete-judges-".$table_location_round_class."', '".$judge_avail."', { expires: 1 });";
						...
						<div id="collapseStep6" class="panel-collapse collapse">
							<div class="panel-body">
								<ul class="list-unstyled">
                                	...
                                	<?php if (session_pref_enabled('prefsEvalAdminTools', 1)) { ?>
                                	<li><a href="<?php echo $base_url; ?>index.php?section=admin&amp;go=evaluation&amp;action=overview&amp;view=admin" data-toggle="tooltip" data-placement="top" title="Track evaluation progress by judging session and table">Evaluation Progress</a></li>
                                	<li><a href="<?php echo $base_url; ?>index.php?section=admin&amp;go=evaluation&amp;action=entrant_tracker&amp;view=admin" data-toggle="tooltip" data-placement="top" title="Track selected entrants' placements for BoS judge eligibility"><?php echo (isset($label_eval_tracker)) ? $label_eval_tracker : "Entrant Placement Tracker"; ?></a></li>
                                	<li><a href="<?php echo $base_url; ?>index.php?section=admin&amp;go=evaluation&amp;action=judge_view&amp;view=admin" data-toggle="tooltip" data-placement="top" title="View all electronic evaluations submitted by a selected judge"><?php echo (isset($label_eval_judge_view)) ? $label_eval_judge_view : "Judge Evaluations"; ?></a></li>
                                	<?php } ?>
                                	<li><a href="<?php echo $base_url; ?>index.php?section=admin&amp;go=evaluation&amp;filter=default&amp;view=admin" data-toggle="tooltip" data-placement="top" title="Manage, View and Edit Judges' evaluations of received entries">Manage Entry Evaluations</a></li>
...
<style>
#judgingTables .judging-table-styles {
    width: 15%;
}
#judgingTables .judging-table-judges {
    width: 30%;
}
#judgingTables .assigned-judge-details li,
#judgingTables .assigned-steward-details li {
    white-space: nowrap;
}
</style>
<script>
 $(document).ready(function() {
    ...
    var locationFilterStorageKey = 'judgingTablesLocationFilter-<?php echo md5($prefix); ?>';

    try {
        var savedLocationFilter = JSON.parse(localStorage.getItem(locationFilterStorageKey));
        if ($.isArray(savedLocationFilter)) {
            $('#judgingTableLocationFilter input.location-filter').each(function() {
                $(this).prop('checked',$.inArray($(this).val(),savedLocationFilter) !== -1);
            });
        }
    }
    catch (error) {
        // Use the default selection when browser storage is unavailable.
    }

    $.fn.dataTableExt.afnFiltering.push(function(settings,data,dataIndex) {
        if (settings.nTable.id != 'judgingTables') return true;
        var row = settings.aoData[dataIndex].nTr;
        var locationID = $(row).attr('data-location-id');
        return $('#judgingTableLocationFilter input.location-filter[value="' + locationID + '"]').is(':checked');
    });
    <?php } ?>

	var judgingTables = $('#judgingTables').dataTable( {
		"bPaginate" : false,
		"sDom": 'fprtp',
		"bStateSave" : false,
		"bLengthChange" : false,
		...
		"aoColumns": [
			...
			<?php if (($totalRows_judging > 1) && ($dbTable == "default"))  { ?>{ "asSorting": [  ] },<?php } ?>
			<?php if ($dbTable == "default") { ?>{ "asSorting": [  ] }<?php } ?>
			]
		} );

        <?php if (!empty($judge_details['by_table'])) { ?>
        $('#toggleAllAssignedJudges').on('click', function(event) {
            event.preventDefault();
            event.stopPropagation();
            var expand = $(this).attr('aria-expanded') !== 'true';
            $('.assigned-judge-details').collapse(expand ? 'show' : 'hide');
            $(this).attr('aria-expanded',expand);
            if (expand) $(this).html('<i class="fa fa-compress"></i> Collapse All');
            else $(this).html('<i class="fa fa-expand"></i> Expand All');
        });
        <?php } ?>

        <?php if (!empty($steward_details['by_table'])) { ?>
        $('#toggleAllAssignedStewards').on('click', function(event) {
            event.preventDefault();
            event.stopPropagation();
            var expand = $(this).attr('aria-expanded') !== 'true';
            $('.assigned-steward-details').collapse(expand ? 'show' : 'hide');
            $(this).attr('aria-expanded',expand);
            if (expand) $(this).html('<i class="fa fa-compress"></i> Collapse All');
            else $(this).html('<i class="fa fa-expand"></i> Expand All');
        });
        ...
        <?php if (($totalRows_judging > 1) && ($dbTable == "default")) { ?>
        $('#judgingTableLocationFilter .dropdown-menu').on('click', function(event) {
            event.stopPropagation();
        });

        function updateTableLocationFilter() {
            var selectedLocations = [];
            $('#judgingTableLocationFilter input.location-filter:checked').each(function() {
                selectedLocations.push($(this).val());
            });
            var selected = $('#judgingTableLocationFilter input.location-filter:checked').length;
            var total = $('#judgingTableLocationFilter input.location-filter').length;
            $('#judgingTableLocationFilterLabel').text('Location (' + selected + ' of ' + total + ')');

            try {
                if (selected == total) localStorage.removeItem(locationFilterStorageKey);
                else localStorage.setItem(locationFilterStorageKey,JSON.stringify(selectedLocations));
            }
            catch (error) {
                // Continue filtering for this visit when browser storage is unavailable.
            }

            judgingTables.fnDraw();
        }

        $('#judgingTableLocationFilter input.location-filter').on('change', function() {
            updateTableLocationFilter();
        });

        $('#selectAllTableLocations').on('click', function(event) {
            event.preventDefault();
            $('#judgingTableLocationFilter input.location-filter').prop('checked',true);
            updateTableLocationFilter();
        });

        $('#clearAllTableLocations').on('click', function(event) {
            event.preventDefault();
            $('#judgingTableLocationFilter input.location-filter').prop('checked',false);
            updateTableLocationFilter();
        });
...
<table class="table table-responsive table-bordered table-striped" id="judgingTables">
    ...
    <tr>
    	<th>#</th>
        <th>Name</th>
        <th class="judging-table-styles">Style(s)</th>
        ...
        <th class="hidden-xs hidden-sm judging-table-judges">
            Judge Assignments
            <?php if (!empty($judge_details['by_table'])) { ?>
            <button type="button" class="btn btn-link btn-xs hidden-print" id="toggleAllAssignedJudges" aria-expanded="false">
        ...
        <th class="hidden-xs hidden-sm">
            Steward Assignments
            <?php if (!empty($steward_details['by_table'])) { ?>
            <button type="button" class="btn btn-link btn-xs hidden-print" id="toggleAllAssignedStewards" aria-expanded="false">
                <i class="fa fa-expand"></i> Expand All
            </button>
            <?php } ?>
        </th>
        <?php if (($totalRows_judging > 1) && ($dbTable == "default"))  { ?>
        <th class="hidden-xs hidden-sm">
            <div class="dropdown" id="judgingTableLocationFilter">
                <button type="button" class="btn btn-link btn-xs dropdown-toggle" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                    <i class="fa fa-filter"></i> <span id="judgingTableLocationFilterLabel">Location (<?php echo count($table_filter_locations); ?> of <?php echo count($table_filter_locations); ?>)</span> <span class="caret"></span>
                </button>
                <ul class="dropdown-menu">
                    <li><a href="#" id="selectAllTableLocations">Select All / Clear Filter</a></li>
                    <li><a href="#" id="clearAllTableLocations">Uncheck All</a></li>
                    <li role="separator" class="divider"></li>
                    <?php foreach ($table_filter_locations as $table_location_id => $table_filter_location) { ?>
                    <li>
                        <label style="display: block; padding: 3px 20px; font-weight: normal; white-space: nowrap;">
                            <input type="checkbox" class="location-filter" value="<?php echo htmlspecialchars($table_location_id,ENT_QUOTES,"UTF-8"); ?>" checked> <?php echo htmlspecialchars($table_filter_location,ENT_QUOTES,"UTF-8"); ?>
                        </label>
                    </li>
                    <?php } ?>
                </ul>
...
#sticky-table-header {
    ...
    -webkit-box-shadow: 0px 4px 6px -6px rgba(0,0,0,0.5);
    -moz-box-shadow: 0px 4px 6px -6px rgba(0,0,0,0.5);
...
#sticky-table-fields .panel-body {
    padding-bottom: 5px;
}
#sticky-table-fields .form-group {
    margin-bottom: 10px;
}
#sticky-table-sidebar {
    display: -webkit-flex;
    display: flex;
    align-items: stretch;
}
...
<div id="sticky-table-header" class="hidden-print">
    <div id="sticky-table-fields" class="panel panel-default">
        <div class="panel-body">
    ...
    <div id="sticky-table-sidebar">
        <div id="sticky-total" class="alert bg-primary">
            <strong>Total for this Table:</strong> <span id="table-total"><?php echo $table_total; ?></span>
        </div>
        <div id="sticky-table-actions" class="well well-sm">
            <div class="checkbox"><label><input type="checkbox" name="return-to-add-table" value="1"> Return here to add another table?</label></div>
