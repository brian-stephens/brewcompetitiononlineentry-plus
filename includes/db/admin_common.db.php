...
// Skip full scores load on routine admin navigation; keep for Manage Scores and non-admin includes (exports/outputs)
if ((check_setup($judging_scores_db_table,$database)) && (($section != "admin") || ($go == "judging_scores"))) {
...
// Fee totals are expensive (historically N+1); only compute where the UI shows them
$total_fees = 0;
$total_fees_paid = 0;
$total_fees_unpaid = 0;
$total_nopay_received = 0;
if (($go == "default") || ($go == "entries")) {
	...
	if ($go == "entries") $total_nopay_received = total_nopay_received($go, "default", $_SESSION['comp_id']);
}
