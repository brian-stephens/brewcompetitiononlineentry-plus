...
/**
 * Entry queries for the current request.
 * Full row sets are loaded only when a view iterates $row_log / $log_*.
 * Everywhere else uses COUNT(*) to avoid materializing the brewing table.
 */

$entries_load_rows = FALSE;

// Sections/views that iterate entry rows
if (
	($section == "list") ||
	($section == "pay") ||
	($section == "brew") ||
	($section == "notes") ||
	(($section == "eval") && ($id != "default")) ||
	(($section == "admin") && ($go == "entries"))
) {
	$entries_load_rows = TRUE;
}
...
elseif ($section == "admin") {
	...
	if ($go == "entries") {
		...
		if ($action == "edit") {
			$query_log = sprintf("SELECT * FROM %s WHERE id = '%s'", $brewing_db_table, $id);
			$query_log_paid = sprintf("SELECT COUNT(*) as 'count' FROM %s WHERE brewPaid='1'", $brewing_db_table);
			$query_log_confirmed = sprintf("SELECT COUNT(*) as 'count' FROM %s WHERE brewConfirmed='1'", $brewing_db_table);
			$entries_load_paid_confirmed_rows = FALSE;
		}
			...
			// Sidebar / unconfirmed math only need totals
			$query_log_paid = sprintf("SELECT COUNT(*) as 'count' FROM %s WHERE brewPaid='1'", $brewing_db_table);
			$query_log_confirmed = sprintf("SELECT COUNT(*) as 'count' FROM %s WHERE brewConfirmed='1'", $brewing_db_table);
			$entries_load_paid_confirmed_rows = FALSE;
		...
		// Non-entries admin pages only need aggregate counts
		$query_log = sprintf("SELECT COUNT(*) as 'count' FROM %s", $brewing_db_table);
		$query_log_paid = sprintf("SELECT COUNT(*) as 'count' FROM %s WHERE brewReceived='1'", $brewing_db_table);
		$query_log_confirmed = sprintf("SELECT COUNT(*) as 'count' FROM %s WHERE brewConfirmed='1'", $brewing_db_table);

		if (SINGLE) {
			$query_log .= sprintf(" WHERE comp_id='%s'", $_SESSION['comp_id']);
			$query_log_paid .= sprintf(" AND comp_id='%s'", $_SESSION['comp_id']);
			$query_log_confirmed .= sprintf(" AND comp_id='%s'", $_SESSION['comp_id']);
		}
...
elseif (($section == "eval") && ($id != "default")) {
	...
	// Public / login / register / default / etc. — counts only
	if ((isset($_SESSION['loginUsername'])) && (isset($_SESSION['user_id'])) && ($section != "admin")) {
		$query_log = sprintf("SELECT COUNT(*) as 'count' FROM %s WHERE brewBrewerID = '%s'", $brewing_db_table, $_SESSION['user_id']);
		$query_log_paid = sprintf("SELECT COUNT(*) as 'count' FROM %s WHERE brewBrewerID = '%s' AND brewReceived='1'", $brewing_db_table, $_SESSION['user_id']);
		$query_log_confirmed = sprintf("SELECT COUNT(*) as 'count' FROM %s WHERE brewBrewerID = '%s' AND brewConfirmed='1'", $brewing_db_table, $_SESSION['user_id']);
	}
	else {
		$query_log = sprintf("SELECT COUNT(*) as 'count' FROM %s", $brewing_db_table);
		$query_log_paid = sprintf("SELECT COUNT(*) as 'count' FROM %s WHERE brewReceived='1'", $brewing_db_table);
...
$use_count_for_paid_confirmed = (isset($entries_load_paid_confirmed_rows) && ($entries_load_paid_confirmed_rows === FALSE));

if ($entries_load_rows) {

	$log = mysqli_query($connection,$query_log) or die (mysqli_error($connection));
	$row_log = mysqli_fetch_assoc($log);
	$totalRows_log = mysqli_num_rows($log);

	if ($use_count_for_paid_confirmed) {
		$log_paid = mysqli_query($connection,$query_log_paid) or die (mysqli_error($connection));
		$row_log_paid_count = mysqli_fetch_assoc($log_paid);
		$totalRows_log_paid = isset($row_log_paid_count['count']) ? (int)$row_log_paid_count['count'] : 0;
		$row_log_paid = NULL;

		$log_confirmed = mysqli_query($connection,$query_log_confirmed) or die (mysqli_error($connection));
		$row_log_confirmed_count = mysqli_fetch_assoc($log_confirmed);
		$totalRows_log_confirmed = isset($row_log_confirmed_count['count']) ? (int)$row_log_confirmed_count['count'] : 0;
		$row_log_confirmed = NULL;
	}
	else {
		$log_paid = mysqli_query($connection,$query_log_paid) or die (mysqli_error($connection));
	...
	$log = mysqli_query($connection,$query_log) or die (mysqli_error($connection));
	$row_log_count = mysqli_fetch_assoc($log);
	$totalRows_log = isset($row_log_count['count']) ? (int)$row_log_count['count'] : 0;
	$row_log = NULL;
...
// Competition-wide entry total (cheap COUNT); reused by constants.inc.php
if ((!$entries_load_rows) && ((($section == "admin") && ($go != "entries")) || ((!isset($_SESSION['loginUsername'])) || (!isset($_SESSION['user_id']))))) {
	$totalRows_entry_count = $totalRows_log;
}
else {
