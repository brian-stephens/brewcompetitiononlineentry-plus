...
// Keep the previous token for one rotation so in-flight forms opened in another tab
// can still be accepted if submitted shortly after a page refresh elsewhere.
$prior_user_session_token = $_SESSION['user_session_token'] ?? '';
if ((is_string($prior_user_session_token)) && (preg_match('/^[a-f0-9]{64}$/i', $prior_user_session_token))) {
    $_SESSION['user_session_token_prev'] = $prior_user_session_token;
}
else {
    unset($_SESSION['user_session_token_prev']);
}
