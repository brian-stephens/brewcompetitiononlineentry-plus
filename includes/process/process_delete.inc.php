...
if ((isset($_SERVER['HTTP_REFERER'])) && ((isset($_SESSION['loginUsername'])) && (isset($_SESSION['userLevel'])))) {
	...
	elseif ($go == "default") {
		...
		// Deleting a judge's evaluation should recompute the entry's average
		// consensus score across the judges that remain.
		elseif ($dbTable == $prefix."evaluation") {

			if ($admin_superuser) {

				$eval_eid = "";
				$query_eval_delete = sprintf("SELECT eid FROM %s WHERE id='%s'", $dbTable, $id);
				$eval_delete = mysqli_query($connection,$query_eval_delete) or die (mysqli_error($connection));
				$row_eval_delete = mysqli_fetch_assoc($eval_delete);
				if ($row_eval_delete) $eval_eid = $row_eval_delete['eid'];
				...
				elseif (!empty($eval_eid)) recompute_eval_consensus($eval_eid);
