...
if ((isset($_SERVER['HTTP_REFERER'])) && (((isset($_SESSION['loginUsername'])) && ((isset($_SESSION['userLevel'])) && ($_SESSION['userLevel'] == 0))) || ($section == "setup"))) {
	...
	if ($go == "default") {
		...
		*/
		...
		if (isset($_POST['contestExtraAck'])) $contestExtraAck = sterilize($_POST['contestExtraAck']);
		$contest_extra_ack_items = array();
		if ((isset($_POST['contestExtraAckTitle'])) && (is_array($_POST['contestExtraAckTitle']))) {
			$ack_titles = $_POST['contestExtraAckTitle'];
			$ack_texts = (isset($_POST['contestExtraAckText']) && is_array($_POST['contestExtraAckText'])) ? $_POST['contestExtraAckText'] : array();
			$ack_checks = (isset($_POST['contestExtraAckCheckbox']) && is_array($_POST['contestExtraAckCheckbox'])) ? $_POST['contestExtraAckCheckbox'] : array();
			$ack_count = count($ack_titles);
			if ($ack_count > 5) $ack_count = 5;
			for ($ack_i = 0; $ack_i < $ack_count; $ack_i++) {
				$ack_title = isset($ack_titles[$ack_i]) ? trim($purifier->purify($ack_titles[$ack_i])) : "";
				$ack_text = isset($ack_texts[$ack_i]) ? trim($purifier->purify($ack_texts[$ack_i])) : "";
				$ack_checkbox = isset($ack_checks[$ack_i]) ? trim($purifier->purify($ack_checks[$ack_i])) : "";
				if (($ack_title === "") && ($ack_text === "") && ($ack_checkbox === "")) continue;
				$contest_extra_ack_items[] = array(
					'title' => $ack_title,
					'text' => $ack_text,
					'checkbox' => $ack_checkbox
				);
			}
		}
		$contestExtraAcks = json_encode($contest_extra_ack_items);
	...
	if ($action == "add") {
		...
		$data = array(
			...
			'contestExtraAck' => $contestExtraAck,
			'contestExtraAcks' => blank_to_null($contestExtraAcks)
		);

		$query_contest_info_exists = sprintf("SELECT COUNT(*) as 'count' FROM %s WHERE id='1'", $contest_info_db_table);
		$contest_info_exists = mysqli_query($connection,$query_contest_info_exists) or die (mysqli_error($connection));
		$row_contest_info_exists = mysqli_fetch_assoc($contest_info_exists);

		if ((!empty($row_contest_info_exists['count'])) && ($row_contest_info_exists['count'] > 0)) {
			$db_conn->where ('id', 1);
