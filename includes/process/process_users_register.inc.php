...
if (isset($_SERVER['HTTP_REFERER'])) {
	...
	$extra_acks_ok = TRUE;
	if (($view == "default") && ($filter != "admin") && (!empty($extra_acks))) {
		$posted_extra_acks = (isset($_POST['brewerExtraAck']) && is_array($_POST['brewerExtraAck'])) ? $_POST['brewerExtraAck'] : array();
		foreach ($extra_acks as $extra_ack_i => $extra_ack) {
			if ((!isset($posted_extra_acks[$extra_ack_i])) || ($posted_extra_acks[$extra_ack_i] != "Y")) {
				$extra_acks_ok = FALSE;
				break;
			}
		}
	...
	elseif (($view == "default") && ($filter != "admin") && (!$extra_acks_ok)) {

		$no_register = TRUE;
		$redirect = $base_url."index.php?section=".$section."&go=".$go."&msg=7";
		$redirect = prep_redirect_link($redirect);
		$redirect_go_to = sprintf("Location: %s", $redirect);

	} // end if (($view == "default") && ($filter != "admin") && (!$extra_acks_ok))
