...
if (isset($_SERVER['HTTP_REFERER'])) {
	...
	if ($action == "email") {
		...
		// Check for spam patterns in both message and subject
		$spam_patterns = [
		    '/\b(viagra|cialis|pharmacy|casino|poker|lottery|bitcoin|crypto|nft|forex|trading|seo|backlink|ranking|adult|xxx|porn|escort)\b/i',
		    '/(http|https|www\.)\S+/i', // Any URL
		    '/\b\d{10,}\b/', // Long number sequences
		    '/[A-Z]{10,}/', // Excessive caps
		    '/(.)\1{5,}/', // Repeated characters (e.g. aaaaaaa)
		    '/\$\s*\d[\d,]*\.\d{2}/', // Dollar amounts
		];

		$spam_count = 0;
		$check_content = (isset($_POST['message']) ? $_POST['message'] : '') . ' ' . (isset($_POST['subject']) ? $_POST['subject'] : '');

		foreach ($spam_patterns as $pattern) {
		    if (preg_match($pattern, $check_content)) {
		        $spam_count++;
		    }
		}
		...
		if ($_SESSION['prefsCAPTCHA'] == 1) {
			...
			if ($captcha_response) {
				...
				if (($response_data->success) && (isset($response_data->hostname)) && ($_SERVER['SERVER_NAME'] == $response_data->hostname)) $captcha_success = TRUE;
