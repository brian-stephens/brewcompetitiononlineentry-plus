		...
		if ($_SESSION['jPrefsQueued'] == "N") {
			...
			foreach ($_POST['random'] as $random) {
				...
				*/
				...
				// Optional move: update their other-table assignment row to this table.
				if (($unassign > 0) && ((isset($_POST['assignFlight'.$random])) && ($_POST['assignFlight'.$random] > 0))) {
				...
				} // end if ((isset($_POST['assignFlight'.$random])) && ($_POST['assignFlight'.$random] == 0))
	  	...
	  	if ($_SESSION['jPrefsQueued'] == "Y") {
			...
			foreach ($_POST['random'] as $random) {
				...
				if (($unassign == 0) && (($_POST['rolesPrevDefined'.$random] == 1) || ($_POST['rolesPrevDefined'.$random] == 0)) && (!empty($assignRoles))) $roles_only_update = TRUE;
				elseif (($unassign == 0) && ($_POST['rolesPrevDefined'.$random] == 1) && (empty($assignRoles))) $roles_only_update = TRUE;

				// Add this table when unassign is unchecked/absent (hidden 0 or checkbox not sent).
				if (($unassign == 0) && ((isset($_POST['assignRound'.$random])) && ($_POST['assignRound'.$random] > 0)))  {

					$query_flights = sprintf("SELECT id FROM %s WHERE (bid='%s' AND assignTable='%s' AND assignRound='%s' AND assignLocation='%s')", $prefix."judging_assignments", $_POST['bid'.$random], $id, $_POST['assignRound'.$random], $_POST['assignLocation'.$random]);
					$flights = mysqli_query($connection,$query_flights) or die (mysqli_error($connection));
					$row_flights = mysqli_fetch_assoc($flights);
					$totalRows_flights = mysqli_num_rows($flights);
				...
				} // end if (($unassign == 0) && ((isset($_POST['assignRound'.$random])) && ($_POST['assignRound'.$random] > 0)))
				...
				// Clear only this table's assignment (preserve other tables in the same round).
				if ((isset($_POST['assignRound'.$random])) && ($_POST['assignRound'.$random] == 0)) {

					$query_flights = sprintf("SELECT id FROM %s WHERE bid='%s' AND assignTable='%s'", $prefix."judging_assignments", $_POST['bid'.$random], $id);
					$flights = mysqli_query($connection,$query_flights) or die (mysqli_error($connection));
					$row_flights = mysqli_fetch_assoc($flights);
					$totalRows_flights = mysqli_num_rows($flights);
				...
				} // end if ((isset($_POST['assignRound'.$random])) && ($_POST['assignRound'.$random] == 0))
