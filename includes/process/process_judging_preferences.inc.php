...
if ((isset($_SERVER['HTTP_REFERER'])) && (((isset($_SESSION['loginUsername'])) && ((isset($_SESSION['userLevel'])) && ($_SESSION['userLevel'] <= 1))) || ($section == "setup"))) {
	...
	if ((!is_null($jPrefsMaxBOS)) && ((int)$jPrefsMaxBOS > 3)) $jPrefsMaxBOS = 3;
	$jPrefsRounds = blank_to_null(sterilize($_POST['jPrefsRounds']));

	// Cap of "0" is intentional (block all signups). Blank means unlimited (NULL).
	// Must use a strict check: in PHP, "" == 0 is true, so a cleared field was
	// previously saved back as 0 and immediately hit the registration limit.
	$raw_cap_stewards = isset($_POST['jPrefsCapStewards']) ? trim((string) $_POST['jPrefsCapStewards']) : '';
	$raw_cap_judges = isset($_POST['jPrefsCapJudges']) ? trim((string) $_POST['jPrefsCapJudges']) : '';
	$jPrefsCapStewards = ($raw_cap_stewards === '0') ? 0 : blank_to_null(sterilize($raw_cap_stewards));
	$jPrefsCapJudges = ($raw_cap_judges === '0') ? 0 : blank_to_null(sterilize($raw_cap_judges));
	...
	if (($action == "edit") || ($section == "setup")) {
		...
		$data = array(
			...
			'prefsEvalAdminTools' => (isset($_POST['prefsEvalAdminTools']) ? sterilize($_POST['prefsEvalAdminTools']) : 0),
		...
		if ($_POST['prefsEval'] == 1) {
			...
			if (!check_update("jPrefsJudgeStats", $prefix."judging_preferences")) {

				$sql = sprintf("ALTER TABLE `%s` ADD `jPrefsJudgeStats` char(1) NULL DEFAULT 'Y' COMMENT 'Track and display judge words/time-per-entry analytics' AFTER `jPrefsScoreDispMax`;
				", $prefix."judging_preferences");
			...
			if (!check_update("evalDurationSec", $prefix."evaluation")) {

				$sql = sprintf("ALTER TABLE `%s` ADD `evalDurationSec` int(10) NULL DEFAULT NULL COMMENT 'Seconds spent on initial scoresheet submission';
				", $prefix."evaluation");
		    ...
		    $query_judging_locations = sprintf("SELECT id, judgingDate, judgingDateEnd FROM %s WHERE judgingLocType <= '1' AND (judgingLocHidden IS NULL OR judgingLocHidden <> '1');", $prefix."judging_locations");
			...
			$jPrefsJudgeStats = (isset($_POST['jPrefsJudgeStats']) ? sterilize($_POST['jPrefsJudgeStats']) : 'Y');
			...
			$data = array(
				...
				'jPrefsJudgeStats' => $jPrefsJudgeStats
			);
			$db_conn->where ('id', $id);
			$result = $db_conn->update ($update_table, $data);
			if (!$result) {
		...
		if ($action == "edit") {
			 ...
			 * convert any previously awarded 4th places to HM (5)
			 * so historical awards align with the new no-4th policy.
			 */
			$sql = sprintf("UPDATE `%s` SET `evalPlace`='5' WHERE `evalPlace`='4';", $prefix."evaluation");
			$db_conn->rawQuery($sql);
			if ($db_conn->getLastErrno() !== 0) {
				$error_output[] = $db_conn->getLastError();
				$errors = TRUE;
			...
			$sql = sprintf("UPDATE `%s` SET `scorePlace`='5' WHERE `scorePlace`='4';", $prefix."judging_scores");
			$db_conn->rawQuery($sql);
			if ($db_conn->getLastErrno() !== 0) {
				$error_output[] = $db_conn->getLastError();
				$errors = TRUE;
			...
			$sql = sprintf("UPDATE `%s` SET `scorePlace`='5' WHERE `scorePlace`='4';", $prefix."judging_scores_bos");
			$db_conn->rawQuery($sql);
			if ($db_conn->getLastErrno() !== 0) {
				$error_output[] = $db_conn->getLastError();
				$errors = TRUE;
		...
		if ($section == "setup") {
			...
			$redirect = $base_url."index.php?msg=22";
