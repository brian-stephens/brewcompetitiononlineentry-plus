...
if ($setup_success) {
	...
	if ((isset($_SESSION['prefsEval'])) && ($_SESSION['prefsEval'] == 1)) {
		...
		if (($section == "evaluation") && (($go == "scoresheet") || ($go == "reconcile"))) {
			
			/**
			 * For usability when accessing an electronic scoresheet 
			 * (or the post-submit reconcile/waiting screen), make sure 
			 * session timeout is at least 30 minutes to give slowpoke 
			 * judges sufficient time for evaluations (typically should 
			 * be no more than 15 minutes per entry).
			 */
